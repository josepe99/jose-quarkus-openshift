...
public class BankDatasource {
    ...
    public Response fetchParametrosSipap(String dominio, String authorizationHeader) {
        LOG.infof("autorizationHeader: %s", authorizationHeader);

        String bearerToken = "Bearer " + jwtToken;

        LOG.infof("Calling middleware with %s.", bearerToken);


        return middlewareApiClient.getParametrosSipap(VERSION_HEADER_VALUE, bearerToken,
                "motivos-sipap");
    }
}
